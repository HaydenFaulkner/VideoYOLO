...
class FlowNetS(HybridBlock):
    ...
    """
    def __init__(self, prefix='flownetS', return_features=False, **kwargs):
    ...
    def hybrid_forward(self, F, x):
        ...
        if self.return_features:
            return concat3, concat4, concat5
...
def get_flownet(version, pretrained=False, ctx=mx.cpu(),
                root=os.path.join('models', 'definitions', 'flownet', 'weights'), **kwargs):

    assert version in ['S', 'C']

    if version == 'S':
        net = FlowNetS(**kwargs)
        pretrained_path = 'FlowNet2-S_checkpoint.params'
    else:
        net = FlowNetC(**kwargs)
        pretrained_path = 'FlowNet2-C_checkpoint.params'
