...
def get_dataloader(net, train_dataset, val_dataset, data_shape, batch_size, num_workers, args):
    ...
    val_loader = gluon.data.DataLoader(
        ...
        batch_size, False, batchify_fn=val_batchify_fn, last_batch='discard', num_workers=num_workers)  # todo this could cause the diff? althought just ran and fine... multi-gpu?
...
def resume(net, async_net, args):
    ...
    if args.start_epoch == -1:
        ...
        files = [file for file in files if '_best' in file]
        files = [file for file in files if '.params' in file]
        files.sort()
        resume_file = files[-1]
        args.start_epoch = 0#int(resume_file[:-7].split('_')[-1]) + 1
