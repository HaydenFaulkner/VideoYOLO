...
"""
given a imagenet vid set, compute mAP modified from FGFA
"""
...
def parse_set(dataset, iou_thr=0.5, pixel_tolerance=10):
    ...
    :param iou_thr: the standard iou thresh to meet, default 0.5
    :param pixel_tolerance: for small objects this will allow a smaller iou to be acceptable to be TP, default 10
    :return: list of dict
    """
    res = []
...
def vid_eval_motion(dataset, dt, motion_ranges, area_ranges, iou_threshold=0.5, class_map=None):
    ...
    :param dataset: an ImageNetVidDetection dataset instance
    :param dt: the detections
    :param motion_ranges: the motion ranges
    :param area_ranges: the area ranges
    :param iou_threshold: the iou threshold necessary for TP
    :return: average precision array with shapes (# motion ranges, # area ranges, # classes)
    """
    classname_map = dataset.wn_classes
    gt_img_ids = dataset.image_ids
    ...
    num_imgs = max(max(gt_img_ids), max(img_ids)) + 1  # todo maybe len not max?
...
class VIDDetectionMetric(mx.metric.EvalMetric):
    """Detection metric for ImageNet VID bbox task.

    Parameters
    ----------
    dataset : instance of ImageNetVidDetectionDetection
        The validation dataset.
    conf_score_thresh : float
        Detection results with confident scores smaller than ``score_thresh`` will
        be discarded before evaluation.
    iou_thresh : float
        The iou threshold necessary for boxes to be considered true positives.
        Note there is also a pixel tolerance for small objects to have a lower iou threshold, see code
    ...
    def get(self):
        ...
        for motion_index, motion_range in enumerate(self._motion_ranges):
            for area_index, area_range in enumerate(self._area_ranges):
                ...
                info_str += 'Mean AP@{:.1f} = {:.4f}\n\n'.format(self._iou_thresh,
                            np.mean([ap[motion_index][area_index][i] for i in range(len(ap[motion_index][area_index]))
